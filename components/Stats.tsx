...
const stats: Stat[] = [
  { num: 50, suffix: '%', label: 'D’économie vs marché français' },
...
export function Stats() {
  return (
    <section id="chiffres" className="py-20 md:py-32 bg-black relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-white/5 rounded-2xl overflow-hidden border border-white/5">
          {stats.map((stat, idx) => (
            <motion.div
              ...
              className="bg-black hover:bg-zinc-950 transition-colors p-4 sm:p-8 md:p-12 flex flex-col items-center md:items-start text-center md:text-left min-w-0"
            >
              {/* text-3xl en base : « 120 000+ » doit tenir dans une demi-colonne de 360px. */}
              <p className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-display font-bold text-white mb-2 md:mb-3 tracking-tighter">
                <CountUp value={stat.num} suffix={stat.suffix} separator={stat.separator} />
              </p>
              <p className="text-[11px] md:text-sm text-zinc-500 uppercase tracking-[0.15em] md:tracking-[0.2em]">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </div>
